...
summary_reg <- function(x, exp = T, digits = 2, pDigits = 4, printToggle = T, quote = F,n1 = 2, n2 = 2, ciFun = confint){
  a <- tableone::ShowRegTable(model = x, exp = exp, digits = digits, pDigits = pDigits, printToggle = printToggle, quote = quote, ciFun = ciFun) %>%
    as.data.frame()
  b <- broom::tidy(x) %>% as.data.frame()
  beta <- b[["estimate"]] %>% as.numeric() %>% sprintf(paste0("%.", digits, "f"), .)
  se <- b[["std.error"]] %>% as.numeric() %>% sprintf(paste0("%.", digits, "f"), .)
  OR95 <- a[["exp(coef) [confint]"]]
